...
const DoctorsPageContent = () => {
    ...
    // ✅ filteredDoctors already contains top-rated doctors from API (no need to filter again)
    const topRatedDoctors = filteredDoctors;

    // Check if there are no filtered results (after applying search/filters)
    const hasNoFilteredResults = !loading && !error && topRatedDoctors.length === 0;
...
const TopDoctors = () => {
    return (
        <DoctorsFilterProvider useTopRated={true}>
            <DoctorsPageContent />
