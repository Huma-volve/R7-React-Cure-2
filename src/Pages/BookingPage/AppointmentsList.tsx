...
const AppointmentsList: React.FC<AppointmentsListProps> = ({ tab, date }) => {
    ...
    useEffect(() => {
        const fetchAppointments = async () => {
            try {
                setLoading(true);
                const response = await axios.get("https://cure-doctor-booking.runasp.net/api/Customer/Booking/PatientBookings", { headers: { Authorization: `Bearer ${token}`, Accept: "*/*" } });
    ...
    const handleReschedule = (id: number) => {
        // حذف الموعد القديم مباشرة
        setAppointments((prev) => prev.filter((app) => app.id !== id));
    };

    const handleConfirmCancel = () => {
        if (cancelId === null) return;
        let appointmentToCancel = appointments.find((app) => app.id === cancelId);
        localStorage.setItem("canceledAppointment", JSON.stringify(appointmentToCancel));
                ...
                {finalAppointments.length > 0 ? (
                    finalAppointments.map((a) => (
                        <AppointmentCard
                            ...
                            onReschedule={handleReschedule} // هنا
                        />
                    ))
                ) : (
